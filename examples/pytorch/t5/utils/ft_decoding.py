...
class FTT5DecodingWeight(object):
    ...
    def load_from_model(self, model, use_moe_cache = False, top_k_experts = None):
        ...
        #     [2, 4, 33]
        # ]
        top_k_experts = top_k_experts[self.pipeline_para_rank]
        # top_3_experts_in_128 = [
        #     # [115, 13, 15],
        #     # [115, 13, 15],
        #     # [84, 63, 89],
        ...
        # top_k_experts = 3
        if use_moe_cache:
            print(f"****************load moe cache****************")
            for i, row in enumerate(top_k_experts):
...
class FTT5Decoding(nn.Module):
    def __init__(self, decoding_weight_list, lib_path, head_num, head_size, inter_size,
                 ...
                 activation_type="relu", tie_word_embeddings=True, adapter_inter_size=0, adapter_norm_position="pre"): #, cache_size=6, use_cache=True):
        super().__init__()
        ...
        print(f"Loaded FasterTransformer library from: {lib_path}")
        # print(torch.classes.FasterTransformer)
        # print(torch.classes.FasterTransformer.T5Decoding)  # 检查是否能找到 T5Decoding
        # print(torch.classes.FasterTransformerT5Decoding)  # 检查是否能找到 FasterTransformerT5Decoding
        ...
        try:
            self.decoding = torch.classes.FasterTransformer.T5Decoding(head_num, head_size, inter_size, mem_d_model,
                                                                       ...
                                                                       moe_layer_index, *decoding_weight_list) #, cache_size, use_cache
        except:
    ...
    def forward(self, beam_width, max_seq_len, top_k, top_p,
                ...
                bad_words_list=None, stop_words_list=None): #, cache_size=6, use_cache=True):
        ...
        results = self.decoding.forward(beam_width, max_seq_len,
                                        ...
                                        bad_words_list, stop_words_list) #, cache_size, use_cache)
        return results
