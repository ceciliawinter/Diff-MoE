                            ...
                            const T5DecodingWeight<T>* decoding_weights)
{
    ...
    // init cache
    float expert_priority[(num_layer_/2)*expert_num_] = {0.0};
    int moe_cache_size = GlobalConfig::instance().cache_size;
    bool use_moe_cache = GlobalConfig::instance().use_moe_cache;
    int fix_cache_size = GlobalConfig::instance().fix_cache_size;
    float max_val = GlobalConfig::instance().max_val;
    float threshold = GlobalConfig::instance().threshold;
    float dec_in_cache = GlobalConfig::instance().dec_in_cache;
    float dec_out_cache = GlobalConfig::instance().dec_out_cache;
    std::vector<std::vector<int>> top_k_experts = GlobalConfig::instance().top_k_experts;
    for (int layer = 0; layer < num_layer_/2; layer++) {
        for (int rank = 0; rank < moe_cache_size; rank++) {
            int expert_id = top_k_experts[layer][rank];
            expert_priority[layer*expert_num_+expert_id] = max_val; 
        }
    }
    ...
    for (int i = 0; i < num_layer_/2; i++) {
        for (int j = 0; j < moe_cache_size; j++) {
            expert_in_cache[i*moe_cache_size+j] = top_k_experts[i][j];
        }
        for (int j = moe_cache_size; j < moe_cache_size; j++) {
            expert_in_cache[i*moe_cache_size+j] = -1;
        }
    }
    for (int step = max_input_length; step <= (int)max_seq_len; step++) {
        ...
        for (uint ite = 0; ite < iteration_num; ++ite) {
            ...
            // std::cout << "+++++++++++++++++++++++expert_priority[0]: " << expert_priority[0] << std::endl;
            decoder_->forward(
                &decoder_output_tensors, &decoder_input_tensors, &decoding_weights->decoder_layer_weights, expert_priority, expert_in_cache, moe_cache_size, fix_cache_size, max_val, threshold, dec_in_cache, dec_out_cache, use_moe_cache, activated_expert_num, iter_num, cache_hit_num);
            // std::cout << "xxxxxxxxxxxxxxxxxxxxxxexpert_priority[0]: " << expert_priority[0] << std::endl;
    ...
    printf("activated_expert_num: %d\n", activated_expert_num);
    printf("iter_num: %d\n", iter_num);
    printf("average expert activated num: %f\n", (float)activated_expert_num/(float)iter_num);
    printf("cache_hit_num: %d\n", cache_hit_num);
    printf("cache hit rate: %f\n", (float)cache_hit_num/(float)activated_expert_num);
