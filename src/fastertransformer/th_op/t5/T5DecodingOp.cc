    ...
    _weights(w)
{
    ...
    if (ft::GlobalConfig::instance().load_from_cpp) {
        ...
        int moe_cache_size = ft::GlobalConfig::instance().cache_size;
        bool use_moe_cache = ft::GlobalConfig::instance().use_moe_cache;
        int fix_cache_size = ft::GlobalConfig::instance().fix_cache_size;
        float max_val = ft::GlobalConfig::instance().max_val;
        float threshold = ft::GlobalConfig::instance().threshold;
        float dec_in_cache = ft::GlobalConfig::instance().dec_in_cache;
        float dec_out_cache = ft::GlobalConfig::instance().dec_out_cache;
        std::vector<std::vector<int>> top_k_experts = ft::GlobalConfig::instance().top_k_experts;
        ...
        //     {115, 13, 15}, {115, 13, 15}, {84, 63, 89}, {32, 55, 22}, {48, 124, 116}, {17, 123, 63}
        // };
        // const int h_top_3_experts_in_128[6][6] = {
        // // {115, 13, 15, 63, 39, 0}, {115, 13, 15, 63, 39, 0}, {84, 63, 89, 26, 31, 0}, {32, 55, 22, 41, 0, 0}, {48, 124, 116, 10, 28, 0}, {17, 123, 63, 72, 116, 0}};
        // // xsum-fine-tuned-0.1-checkpoint-1596
        //     {43,116,24,0,0,0},
        //     {87,66,125,112,107,22},
        //     {93,41,53,67,10,25},
        //     {88,68,76,95,64,0},
        //     {56,67,118,80,79,52},
        //     {29,74,105,50,81,47}
        // };
        // auto moe_cache_size = 6; //TODO cache_size
            ...
            auto _moe_load_helper = [&]() {
                if (use_moe) {
                    if (disk_offload) {
                        ...
                        if(use_moe_cache) {
                            std::vector<std::string> wi_filenames, wo_filenames;
                            for (int e = 0; e < moe_cache_size; e++) {
                                wi_filenames.push_back(
                                    offload_path + "decoder.block." + std::to_string(i) + ".layer.2.mlp.experts.expert_" + std::to_string(top_k_experts[i/2][e]) + ".wi.weight.0.bin");
                                wo_filenames.push_back(
                                    offload_path + "decoder.block." + std::to_string(i) + ".layer.2.mlp.experts.expert_" + std::to_string(top_k_experts[i/2][e]) + ".wo.weight.0.bin");
                            }
                            _weights.push_back(
                                create_tensor_from_bin<int8_t>(
                                    {moe_cache_size, intermediate_w_size}, wi_filenames, true));
                            auto ptr = get_ptr<int8_t>(_weights[_weights.size() - 1]);
                            decoder_layer_weights->ffn_weights.intermediate_weight_stay_on_GPU.kernel = reinterpret_cast<const T*>(ptr);
                            _weights.push_back(
                                create_tensor_from_bin<int8_t>(
                                    {moe_cache_size, output_w_size}, wo_filenames, true));
                            ptr = get_ptr<int8_t>(_weights[_weights.size() - 1]);
