...
template<typename T>
std::future<void> MemoryArena<T>::allocate(const tag_t&                                tag,
                                           T*                                          dst,
                                           const T*                                    src,
                                           std::function<void(const T*, cudaStream_t)> post_callback)
{
    ...
    auto future = pool_->push([=](int) {
        if (!GlobalConfig::instance().use_cache  // if not use_cache, do this anyway
            ...
            check_cuda_error(
                cudaMemcpyAsync(repl.first, cpy_src, chunk_size_ * sizeof(T), cudaMemcpyHostToDevice, stream_));
        }
...
template<typename T>
std::future<void> MemoryArena<T>::move_cache(const T* src, std::function<void(const T*, cudaStream_t)> post_callback)
{
    auto future = pool_->push([=](int) { post_callback(src, stream_); });
    return future;
}
