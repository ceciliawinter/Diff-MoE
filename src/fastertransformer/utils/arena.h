...
public:
    ...
    std::future<void> allocate(const tag_t&                                tag,
                               T*                                          dst           = nullptr,
                               const T*                                    src           = nullptr,
                               std::function<void(const T*, cudaStream_t)> post_callback = nullptr);

    // move data to cache
    std::future<void> move_cache(const T*                                    src,
...
public:
    ...
    std::future<void> cache_move(char* dsts, const char* src = nullptr, const size_t data_size = 0)
    {
        FT_CHECK_WITH_INFO(arena_ != nullptr, "Memory arena uninitialized.");
        auto post_callback = [dsts = dsts, data_size = data_size](const char* cached_ptr, cudaStream_t stream) {
            const char* ptr = cached_ptr;
            check_cuda_error(cudaMemcpyAsync(dsts, ptr, data_size, cudaMemcpyDeviceToDevice, stream));
        };
        return arena_->move_cache(src, post_callback);
    }
